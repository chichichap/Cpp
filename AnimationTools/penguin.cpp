/***********************************************************
             CSC418, FALL 2012, A2 part B
 
                 penguin.cpp
                 author: Charuvit Wannissorn (1000149341)
...
/** 
 Added Notes: This file was editted using Visual Studio 2010

 Description: opens 4 windows:
	1. The main display window: shows the rendered 3d penguin
	2. Keyframe Controle: shows the GUI that allows the user to do animating stuff
	3. Joint Control: shows the GUI that allows the user to manipulate the penguin
	4. Render Control: shows the GUI that allows the user to choose different rendering styles
**/
...
void updateKeyframeButton(int)
{
	...
	if (maxValidKeyframe < keyframeID)
		maxValidKeyframe = keyframeID;

	// Update the appropriate entry in the 'keyframes' array with the 'joint_ui_data' data
	keyframes[keyframeID].setID(keyframeID);
	keyframes[keyframeID].setTime(joint_ui_data->getTime());
	keyframes[keyframeID].setDOF(0, joint_ui_data->getDOF(Keyframe::ROOT_TRANSLATE_X));
	keyframes[keyframeID].setDOF(1, joint_ui_data->getDOF(Keyframe::ROOT_TRANSLATE_Y));
	keyframes[keyframeID].setDOF(2, joint_ui_data->getDOF(Keyframe::ROOT_TRANSLATE_Z));
	keyframes[keyframeID].setDOF(3, joint_ui_data->getDOF(Keyframe::ROOT_ROTATE_X));
	keyframes[keyframeID].setDOF(4, joint_ui_data->getDOF(Keyframe::ROOT_ROTATE_Y));
	keyframes[keyframeID].setDOF(5, joint_ui_data->getDOF(Keyframe::ROOT_ROTATE_Z));
	keyframes[keyframeID].setDOF(6, joint_ui_data->getDOF(Keyframe::HEAD));
	keyframes[keyframeID].setDOF(7, joint_ui_data->getDOF(Keyframe::R_SHOULDER_PITCH));
	keyframes[keyframeID].setDOF(8, joint_ui_data->getDOF(Keyframe::R_SHOULDER_YAW));
	keyframes[keyframeID].setDOF(9, joint_ui_data->getDOF(Keyframe::R_SHOULDER_ROLL));
	keyframes[keyframeID].setDOF(10, joint_ui_data->getDOF(Keyframe::L_SHOULDER_PITCH));
	keyframes[keyframeID].setDOF(11, joint_ui_data->getDOF(Keyframe::L_SHOULDER_YAW));
	keyframes[keyframeID].setDOF(12, joint_ui_data->getDOF(Keyframe::L_SHOULDER_ROLL));
	keyframes[keyframeID].setDOF(13, joint_ui_data->getDOF(Keyframe::R_HIP_PITCH));
	keyframes[keyframeID].setDOF(14, joint_ui_data->getDOF(Keyframe::R_HIP_YAW));
	keyframes[keyframeID].setDOF(15, joint_ui_data->getDOF(Keyframe::R_HIP_ROLL));
	keyframes[keyframeID].setDOF(16, joint_ui_data->getDOF(Keyframe::L_HIP_PITCH));
	keyframes[keyframeID].setDOF(17, joint_ui_data->getDOF(Keyframe::L_HIP_YAW));
	keyframes[keyframeID].setDOF(18, joint_ui_data->getDOF(Keyframe::L_HIP_ROLL));
	keyframes[keyframeID].setDOF(19, joint_ui_data->getDOF(Keyframe::BEAK));
	keyframes[keyframeID].setDOF(20, joint_ui_data->getDOF(Keyframe::R_ELBOW));
	keyframes[keyframeID].setDOF(21, joint_ui_data->getDOF(Keyframe::L_ELBOW));
	keyframes[keyframeID].setDOF(22, joint_ui_data->getDOF(Keyframe::R_KNEE));
	keyframes[keyframeID].setDOF(23, joint_ui_data->getDOF(Keyframe::L_KNEE));
...
void initGlui()
{
	...
	///////////////////////////////////////////////////////////
	// TODO3 (for controlling light source position & additional
	//      rendering styles):
	...
	///////////////////////////////////////////////////////////
	glui_panel = glui_joints->add_panel("Light Source Location");

	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "light angle:", GLUI_SPINNER_FLOAT, &light_angle);
...
void display(void)
{
	...
	float x=0.5; /* design scale --edit: more like radius */

	// determine render style and set glPolygonMode appropriately
	if (renderStyle == WIREFRAME) 	 glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	else if (renderStyle == SOLID)   glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	else if (renderStyle == OUTLINED) glEnable(GL_POLYGON_OFFSET_FILL);
	else if (renderStyle == METALLIC || MATTE) {
	...
	glPushMatrix(); // push camera transformation matrix on the stack
	glPushMatrix(); // push world transformation matrix on the stack 
		...
		glPushMatrix();
			...
			glPushMatrix();
				...
				// Lower Beak
				glTranslatef(0, -0.5*x*(joint_ui_data->getDOF(Keyframe::BEAK) + 1), 0);
				glScalef(x, 0.25*x, x);
				glColor3f(1.0, 0.8, 0.1);
				drawCube();
			glPopMatrix();
	
			glScalef(1.8*x, 1.8*x, 1.8*x);
			glColor3f(0.5, 0.3, 0.1);
			drawSphere();
		
		glPopMatrix(); // back to center of body

		// Left Arm
	    glPushMatrix();
			glTranslatef(0, 3*x, x); // Left Shoulder Joint Hinge

			// the arm must be sticking out of the body (pitch - 45)
			glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_PITCH)-45, 1.0, 0.0, 0.0);
			...
			// Left Hand
			glPushMatrix();
				glTranslatef(0, -3*x, 0.5*x);
				glRotatef(-joint_ui_data->getDOF(Keyframe::L_ELBOW), 0.0, 0.0, 1.0);
				glPushMatrix();
					glTranslatef(0, -2*x, 0);
					glScalef(1.5*x, 1.5*x, 1.5*x); // draw a spheric tiny hand
					glColor3f(0.5, 0.3, 0.1);
					drawSphere();
				glPopMatrix();
				glTranslatef(0, -x, 0);
				glScalef(0.25*x, x, 0.25*x);
				glColor3f(1.0, 0.0, 0.0);
				drawCube();
			glPopMatrix();

			glTranslatef(0, -1.5*x, 0.5*x);
			glScalef(0.5*x, 1.5*x, 0.5*x);
		...
		// Right Arm
	    glPushMatrix();
			glTranslatef(0, 3*x, -x); // Right Shoulder Joint Hinge

			// the arm must be sticking out of the body (pitch + 45)
			glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_PITCH)+45, 1.0, 0.0, 0.0);
			...
			glPushMatrix();
				glTranslatef(0, -3*x, -0.5*x);
				glRotatef(360-joint_ui_data->getDOF(Keyframe::R_ELBOW), 0.0, 0.0, 1.0);
				glPushMatrix();
					glTranslatef(0, -2*x, 0);
					glScalef(1.5*x, 1.5*x, 1.5*x); // draw a spheric tiny hand
					glColor3f(0.5, 0.3, 0.1);
		...
		// Left Leg
		glPushMatrix();
			glTranslatef(0, -4*x, 0.8*x); // Left Hip Joint Hinge
			...
			glPushMatrix();
				...
				glTranslatef(-x, 0, 0);
				glScalef(2*x, 0.2*x, 0.6*x);
				glColor3f(1.0, 0.8, 0.1);
				drawCube();
			glPopMatrix();
			
			glTranslatef(0, -x, 0);
			glScalef(0.2*x, x, 0.2*x);
			glColor3f(1.0, 0.5, 0.0);
			drawCube();
		glPopMatrix(); // back to center of body
		...
		// Right Leg
		glPushMatrix();
			glTranslatef(0, -4*x, -0.8*x); // Right Hip Joint Hinge
	...
	glPopMatrix(); // retrieve world transformation matrix
    glPopMatrix(); // retrieve camera transformation matrix
	
	if (renderStyle == OUTLINED) glDisable(GL_POLYGON_OFFSET_FILL);
	if (renderStyle == METALLIC || MATTE) { 
...
void setLightning(void) {
	...
	GLfloat LightPosition[] = {cos(light_angle*PI/180)*5, sin(light_angle*PI/180)*5, 10.0, 0.0}; // radius = 5

	GLfloat DiffuseMaterial[4] = {0.1, 0.5, 0.8, 1.0};
	GLfloat AmbientMaterial[4] = {0.7, 0.7, 0.7, 1.0};
	GLfloat SpecularMaterial[4] =  {1.0f, 1.0f, 1.0f, 1.0f};
	GLfloat shine = 0.4;
...
// IT'S NOT A UNIT CUBE FOR GOD'S SAKE!!
