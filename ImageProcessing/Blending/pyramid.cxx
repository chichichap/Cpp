...
pyramid::pyramid(const vil_image_view<vxl_byte>& im) {
	a_ = 0.4; // default value for the alpha parameter
	build(im);
}
...
// main routine for building the Gauss and the Laplacian pyramid
//
// It takes as input an image (of arbitrary dimensions MxP) and builds a pyramid whose 0 level is of size (2^N+1)x(2^N+1)
...
// the kernel always has length 5 pixels
	w_hat_ = new double[5];
	// shift pointer by two so that kernel indices are in the range [-2,2]
	w_hat_ += 2;
...
// a pyramid is represented as an array of N_+1 images, 0,...,N_ where the n-th image is of dimension (2^n+1)x(2^n+1)
...
// Create a pyramid data structure from a sequence of Laplacian pyramid levels and the N-th level Gauss image
				   ...
				   int N, double a)
{
	...
	// the kernel always has length 5 pixels
	w_hat_ = new double[5];
	w_hat_ += 2; // shift pointer by two so that kernel indices are in the range [-2,2]

	w_hat_[2] = w_hat_[-2] = 0.25 - a_/2; // set the values of the kernel
	w_hat_[1] = w_hat_[-1] = 0.25;
	w_hat_[0] = a_;
...
// Copy level l of the Laplacian pyramid into the supplied image; the routine returns FALSE if (l is not in the range [0,...,N_-1]
...
// Copy all levels of the Laplacian pyramid into an array of images and return a pointer to that array
...
// a helper function that adds two images and also performs intensity bound checking to avoid overflows when assigning numbers to unsigned byte images
...
// the N_-th level of the Gauss pyramid is only level stored in the pyramid data structure
	g_temp[N_] = g_N_;

// the remaining levels have to be computed
	for (int l=N_; l>0; l--) {
		vil_image_view<vxl_byte> gtmp;
// compute the l-th level of the Gauss pyramid using the formula g_(l-1) = expand(g_l) + L_[l-1]
...
// Take an image of (2^N+1)x(2^N+1), return an image of (2^(N-1)+1)x(2^(N-1)+1) that is a smoothed and subsampled version of the original
// 
// Parameters:
//    im:     the input image
//    w_hat:  the 5-pixel smoothing kernel (w_hat[-2],...,w_hat[2])
//
// Boundary treatment: When the kernel extends beyond the image boundary, you should compute the convolution
//                     (1) by taking into account only the kernel elements that overlap with the image
//                 and (2) by dividing the result by the sum of these kernel elements

void pyramid::reduce(	const vil_image_view<vxl_byte> 	im,
			            const double* 					w_hat,
							  vil_image_view<vxl_byte>& im_red)
{
	int i,j,p, size = im.ni(), np = im.nplanes();
	double sum;
	double   weight = w_hat[-2] + w_hat[-1] + w_hat[0] + w_hat[1] + w_hat[2];
	double h1weight = w_hat[-2] + w_hat[-1] + w_hat[0];
	double h2weight = 						   w_hat[0] + w_hat[1] + w_hat[2];
	vil_image_view<vxl_byte>* temp = new vil_image_view<vxl_byte>;

	temp->set_size(size, size/2 +1, np);
	im_red.set_size(size/2 +1, size/2 +1, np);

	for (p=0; p < np; p++) {
// step 1: convolve the 1D hat across rows
		for (i=0; i < size; i++) {

// j = 0
			(*temp)(i,0, p) = (w_hat[0]*im(i,0, p) + w_hat[1]*im(i,1, p) + w_hat[2]*im(i,2, p)) / h2weight;

// j = [2 ... size-3]
			for (j=2; j < size-2; j += 2)
				(*temp)(i,j/2, p) = (w_hat[-2]*im(i,j-2, p) + w_hat[-1]*im(i,j-1, p) + w_hat[0]*im(i,j, p) + w_hat[1]*im(i,j+1, p) + w_hat[2]*im(i,j+2, p)) / weight;

// j = size-1
			(*temp)(i,size/2, p) = (w_hat[-2]*im(i,j-2, p) + w_hat[-1]*im(i,j-1, p) + w_hat[0]*im(i,j, p)) / h1weight;
		}

// step 2: convolve the 1D hat across columns
		for (j=0; j < size/2 +1; j++) {

// i = 0
			im_red(0,j, p) = (w_hat[0]*(*temp)(0,j, p) + w_hat[1]*im(1,j, p) + w_hat[2]*im(2,j, p)) / h2weight;

// i = [2 ... size-3]
			for (i=2; i < size-2; i += 2)
				im_red(i/2,j, p) = (w_hat[-2]*(*temp)(i-2,j, p) + w_hat[-1]*(*temp)(i-1,j, p) + w_hat[0]*(*temp)(i,j, p) + w_hat[1]*(*temp)(i+1,j, p) + w_hat[2]*(*temp)(i+2,j, p)) / weight;

// i = size-1
			im_red(size/2,j, p) = (w_hat[-2]*(*temp)(i-2,j, p) + w_hat[-1]*(*temp)(i-1,j, p) + w_hat[0]*(*temp)(i,j, p)) / h1weight;
		}
	}
}

// Take an image of (2^(N-1)+1)x(2^(N-1)+1), return an image of (2^N+1)x(2^N+1) that is an interpolated version of the original
// using the w_hat as the interpolating kernel
// 
// Parameters:
//    im:     the input image
//    w_hat:  the 5-pixel smoothing kernel (w_hat[-2],...,w_hat[2])
						      ...
						      vil_image_view<vxl_byte>& 	im_exp)
{
	int i,j,p, size = im.ni(), np = im.nplanes();
	double sum;
	double   weightA = w_hat[-2] + w_hat[0] + w_hat[2];
	double   weightB = w_hat[-1] + w_hat[1];
	vil_image_view<vxl_byte>* temp = new vil_image_view<vxl_byte>;

	temp->set_size(size, 2*size -1, np);
	im_exp.set_size(2*size -1, 2*size -1, np);
...
// j = 0
			(*temp)(i,0, p) =  (w_hat[0]*im(i,0, p) + w_hat[2]*im(i,1, p)) / (w_hat[0] + w_hat[2]);
			(*temp)(i,1, p) = (w_hat[-1]*im(i,0, p) + w_hat[1]*im(i,1, p)) / weightB;

// j= [1 ... size-2]
			for (j=1; j < size-1; j++) {
				(*temp)(i,2*j,	 p) = (w_hat[-2]*im(i,j-1, p) + w_hat[0]*im(i,j, p) + w_hat[2]*im(i,j+1, p)) / weightA;
				(*temp)(i,2*j+1, p) = (					 	 w_hat[-1]*im(i,j, p) + w_hat[1]*im(i,j+1, p)) / weightB;
			}

// j = size -1;
			(*temp)(i,2*j, p) = (w_hat[-2]*im(i,j-1, p) + w_hat[0]*im(i,j, p)) / (w_hat[-2] + w_hat[0]);
		}
...
// i=0
			im_exp(0,j, p) =  (w_hat[0]*(*temp)(0,j, p) + w_hat[2]*(*temp)(1,j, p)) / (w_hat[0] + w_hat[2]);
			im_exp(1,j, p) = (w_hat[-1]*(*temp)(0,j, p) + w_hat[1]*(*temp)(1,j, p)) / weightB;

// i= [1 ... size-2]
			for (i=1; i < size-1; i++) {
					im_exp(2*i,  j, p) = (w_hat[-2]*(*temp)(i-1,j, p) + w_hat[0]*(*temp)(i,j, p) + w_hat[2]*(*temp)(i+1,j, p)) / weightA;
					im_exp(2*i+1,j, p) = (					    	   w_hat[-1]*(*temp)(i,j, p) + w_hat[1]*(*temp)(i+1,j, p)) / weightB;
			}

// i = size -1;
			im_exp(2*i,j, p) = (w_hat[-2]*(*temp)(i-1,j, p) + w_hat[0]*(*temp)(i,j, p)) / (w_hat[-2] + w_hat[0]);
		}
	}
}
							   ...
							   vil_image_view<int>& 	im_exp)
{
	...
	double   weightA = w_hat[-2] + w_hat[0] + w_hat[2];
	double   weightB = w_hat[-1] + w_hat[1];
	vil_image_view<int>* temp = new vil_image_view<int>;
