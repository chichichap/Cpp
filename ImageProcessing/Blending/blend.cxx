...
bool blend2(
		...
		const vil_image_view<vxl_byte>& mask,
			   vil_image_view<vxl_byte>& result)
{
// step 0: make sure image dimensions match
	...
	pyramid A = pyramid(source0);
	pyramid B = pyramid(source1);
	pyramid M = pyramid(mask);

	int n = A.N(); // number of levels
	int h, w, np = source0.nplanes();
	double a = A.a();

// step 1: compute the Laplacian pyramid for the result S
	vil_image_view<int> *AL = A.L();
	vil_image_view<int> *BL = B.L();
	vil_image_view<vxl_byte> *MG = M.g();
	vil_image_view<int> SL[n];

	for (int k=0; k < n; k++) {
		h = MG[k].ni();
		w = MG[k].nj();

		SL[k].set_size(h, w, np);

		for (int p=0; p < np; p++)
			for (int i=0; i < h; i++)
				for (int j=0; j < w; j++)
					SL[k](i,j,p) = (1 - MG[k](i,j)/255.0) * (double)AL[k](i,j,p)
							     +      MG[k](i,j)/255.0  * (double)BL[k](i,j,p);
	}

// step 2: compute the N-th level of the Gaussian pyramid
	vil_image_view<vxl_byte> AG_N;
	vil_image_view<vxl_byte> BG_N;
	vil_image_view<vxl_byte> SG_N;
	...
	SG_N.set_size(h, w, np);

	for (int p=0; p < np; p++)
		for (int i=0; i < h; i++)
			for (int j=0; j < w; j++)
				SG_N(i,j,p) = (1 - MG[n](i,j)/255.0) * AG_N(i,j,p)
							+ 	   MG[n](i,j)/255.0  * BG_N(i,j,p);

// step 3: reconstitute the full-resolution image for S by computing S = SG_0
	pyramid S = pyramid(SL, SG_N, n, a);
	S.g(0, result);
	return true;
}
...
// The routine just calls the blend() function after checking that all inputs to that function have been specified
